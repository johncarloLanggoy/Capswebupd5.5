...
// ── DATA ────────────────────────────────────────────────────────
const days = [
    { value: 0, label: 'Monday', icon: '📅' },    // 0 = Monday ✅
    { value: 1, label: 'Tuesday', icon: '📅' },   // 1 = Tuesday ✅
    { value: 2, label: 'Wednesday', icon: '📅' }, // 2 = Wednesday ✅
    { value: 3, label: 'Thursday', icon: '📅' },  // 3 = Thursday ✅
    { value: 4, label: 'Friday', icon: '📅' },    // 4 = Friday ✅
    { value: 5, label: 'Saturday', icon: '🎉' },  // 5 = Saturday ✅
    { value: 6, label: 'Sunday', icon: '🌞' }     // 6 = Sunday ✅
];
const timeSlots = [
...
async function loadAvailability() {
    try {
        ...
        showToast('Error loading availability settings', 'error');
    }
}
...
function renderTable() {
    ...
    days.forEach(day => {
        ...
        dayTd.innerHTML = `<div class="day-cell"><span class="day-icon">${day.icon}</span> ${day.label} <span class="day-status-badge ${isOpen ? 'open' : 'closed'}">${isOpen ? 'Open' : 'Closed'}</span>${hasClosure ? '<span class="closure-indicator">📅 Has Closure</span>' : ''}</div>`;
        ...
        toggleTd.innerHTML = `
            <div style="display:flex; align-items:center; justify-content:center; gap:8px; flex-wrap:wrap;">
                ...
                <button onclick="openSpecificDateModal(${day.value}, ${isOpen})" 
                        style="background:rgba(56,189,248,0.1); border:1px solid #334155; border-radius:6px; color:#94a3b8; padding:4px 8px; font-size:11px; cursor:pointer; transition:0.3s;"
                        onmouseover="this.style.color='#38bdf8'; this.style.borderColor='#38bdf8';"
...
async function toggleDay(day, isOpen) {
    ...
    } catch (error) { showToast('❌ Error updating day', 'error'); }
}
...
async function updateMaxSlots(day, value) {
    try {
        ...
        showToast('✅ Daily max slots updated!', 'success');
    } catch (error) { showToast('❌ Error updating max slots', 'error'); }
}
...
function openSlotPopup(day, time) {
    ...
    document.getElementById('popupDayLabel').textContent = `⏰ ${dayLabel} - ${time}`;
...
async function saveSlotSettings() {
    ...
    try {
        ...
        if (data.success) {
            ...
            renderTable(); closeSlotPopup(); showToast('✅ Time slot updated successfully!', 'success');
        } else { showToast('❌ ' + data.message, 'error'); }
    } catch (error) { showToast('❌ Error saving time slot', 'error'); }
}
...
async function saveAllSettings() {
    ...
    btn.disabled = false; btn.innerHTML = '💾 Save All Settings';
}
...
async function applySpecificDateOverride() {
    ...
    if(action === 'close') {
        ...
        if(closureToDelete) {
            ...
            if(delData.success) { 
                showToast(`✅ ${date} reopened!`, 'success'); 
                closeSpecificDateModal(); 
                loadAvailability(); 
            } else { 
        ...
        const data = await res.json();
        if(data.success) { 
            showToast(`✅ Slots for ${date} set to ${maxSlots}!`, 'success'); 
            closeSpecificDateModal(); 
            loadAvailability(); 
        } else { 
            showToast(data.message, 'error'); 
